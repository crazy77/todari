...
import { gameSettingsAtom, defaultSoloSettings, defaultSpeedSettings } from '@/stores/modeAtom';
import { gameEventBus, GameEvents } from '@/game/events';
  ...
  return (
    <div style={{ position: 'fixed', top: 12, right: 12, display: 'flex', gap: 8 }}>
      <button
        ...
        style={{ padding: '6px 10px', background: settings.mode === 'solo' ? '#444' : '#222', color: '#fff' }}
      >
        솔로
      </button>
      <button
        onClick={() => setSettings(defaultSpeedSettings)}
        style={{ padding: '6px 10px', background: settings.mode === 'speed' ? '#444' : '#222', color: '#fff' }}
      >
        스피드배틀
      </button>
      <button
