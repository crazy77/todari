...
export function ShareButtons({ roomId, score }: { roomId: string; score: number }): JSX.Element {
  ...
  return (
    <div style={{ display: 'flex', gap: 8 }}>
      <button onClick={async () => {
        if (navigator.share) {
          try { await navigator.share({ title: 'Todari', text: shareText, url: shareUrl }); } catch {}
          return;
        }
