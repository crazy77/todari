...
import { roomsAtom, settingsAtom, logsAtom, logsLevelAtom, logsLimitAtom, blockedUsersAtom, } from '@/stores/adminAtoms';
...
export function AdminDashboard() {
    ...
    return (_jsxs("div", { className: "min-h-[100svh] w-full p-4 sm:p-6", children: [_jsxs("div", { className: "mb-4 flex items-center justify-between", children: [_jsx("h2", { className: "text-xl font-extrabold text-slate-800", children: "\uAD00\uB9AC\uC790 \uB300\uC2DC\uBCF4\uB4DC" }), _jsx("span", { className: cn('chip', connected ? 'ring-emerald-300' : 'ring-rose-300'), children: connected ? '소켓 연결됨' : '소켓 해제됨' })] }), _jsxs("div", { className: "grid grid-cols-1 gap-4 md:grid-cols-2", children: [_jsxs("section", { className: "card p-4", children: [_jsx("div", { className: "mb-2 text-base font-bold text-slate-700", children: "\uAC8C\uC784\uBC29 \uC0C1\uD0DC" }), _jsx("input", { className: "mb-2 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-brand-primary", placeholder: "room id", value: roomId, onChange: (e) => setRoomId(e.target.value) }), _jsxs("div", { className: "text-sm text-slate-600", children: ["\uC0C1\uD0DC: ", roomStatus] }), _jsxs("div", { className: "mt-3 flex gap-2", children: [_jsx("button", { type: "button", className: "btn-primary", onClick: () => socket.emit('game-start', { roomId }), children: "\uC2DC\uC791" }), _jsx("button", { type: "button", className: "btn-ghost", onClick: () => socket.emit('game-end', { roomId }), children: "\uC885\uB8CC" }), _jsx("button", { type: "button", className: "btn-ghost", onClick: () => socket.emit('leave-room', { roomId }), children: "\uD1F4\uC7A5" })] })] }), _jsxs("section", { className: "card p-4", children: [_jsx("div", { className: "mb-2 text-base font-bold text-slate-700", children: "\uAC8C\uC784 \uC124\uC815" }), _jsxs("form", { className: "grid grid-cols-2 gap-3", onSubmit: async (e) => {
                                    e.preventDefault();
                                    ...
                                    if (data?.settings)
                                        ...
                                        ['baseScore', '기본 점수'],
                                        ['timeBonus', '시간 보너스'],
                                    ]).map(([k, label]) => (_jsxs("label", { className: "text-sm", children: [_jsx("span", { className: "mb-1 block text-slate-600", children: label }), _jsx("input", { type: "number", value: settings[k] ?? '', onChange: (e) => setSettings((s) => ({ ...s, [k]: Number(e.target.value) })), className: "w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-brand-primary" })] }, k))), _jsx("div", { className: "col-span-2", children: _jsx("button", { type: "submit", className: "btn-primary", children: "\uC800\uC7A5" }) })] })] }), _jsxs("section", { className: "card p-4", children: [_jsxs("div", { className: "mb-2 flex items-center justify-between", children: [_jsx("div", { className: "text-base font-bold text-slate-700", children: "\uAC8C\uC784\uBC29 \uBAA9\uB85D" }), _jsx("button", { type: "button", className: "btn-ghost", onClick: async () => {
                                            ...
                                            setRooms((data.rooms ?? []));
                                                    ...
                                                    const data = await res.json();
                                                    setLogs((data.logs ?? []));
                                                }, children: "\uBD88\uB7EC\uC624\uAE30" })] })] }), _jsx("div", { className: "grid max-h-64 gap-1 overflow-auto rounded-lg border border-slate-200 bg-white p-2 text-xs", children: logs.map((l) => (_jsxs("div", { className: cn('rounded-md px-2 py-1', l.level === 'error' ? 'bg-rose-50 text-rose-700' : l.level === 'warn' ? 'bg-amber-50 text-amber-700' : 'bg-slate-50 text-slate-700'), children: [_jsx("span", { className: "mr-2 inline-block w-12 text-[11px] font-bold uppercase", children: l.level }), _jsx("span", { className: "text-[11px] text-slate-500", children: new Date(l.ts).toLocaleTimeString() }), _jsx("span", { className: "ml-2", children: l.message })] }, `${l.ts}-${l.message}`))) })] }), _jsxs("section", { className: "card p-4", children: [_jsxs("div", { className: "mb-2 flex items-center justify-between", children: [_jsx("div", { className: "text-base font-bold text-slate-700", children: "\uCC28\uB2E8 \uAD00\uB9AC" }), _jsx("button", { type: "button", className: "btn-ghost", onClick: async () => {
                                            ...
                                            const data = await res.json();
                                            setBlocked((data.users ?? []));
                                        }, children: [_jsx("input", { name: "uid", placeholder: "user id", className: "w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-brand-primary" }), _jsx("button", { type: "submit", className: "btn-primary", children: "\uCC28\uB2E8" })] }), _jsx("div", { className: "grid gap-1", children: blocked.map((u) => (_jsxs("div", { className: "flex items-center justify-between rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700", children: [_jsx("span", { children: u }), _jsx("button", { type: "button", className: "btn-ghost", onClick: async () => {
